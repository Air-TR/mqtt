...
import com.tr.mqtt.callback.MqttProviderCallBack;
...
public class MqttProviderConfig {
    ...
    public void connect() {
        try {
            // 创建 MQTT 客户端对象
            client = new MqttClient(hostUrl, providerClientId, new MemoryPersistence());
            // 连接设置
            MqttConnectOptions options = new MqttConnectOptions();
            // 是否清空 session，设置 false 表示服务器会保留客户端的连接记录，客户端重连之后能获取到服务器在客户端断开连接期间推送的消息
            // 设置为 true 表示每次连接服务器都是以新的身份
